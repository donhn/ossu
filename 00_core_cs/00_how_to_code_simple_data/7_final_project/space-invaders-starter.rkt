...
;;   2. Updating invader positions and invade countdown
;;   3. Updating missiles positions 
;;   4. Updating tank position

;(define (process-game-state s) ...) ;stub
...
;; Game -> Game
;; Updates the positions of all game objects and spawns additional invaders if countdown reaches 0.
...
;; Game -> Game
;; Process collisions between missiles and invaders.

;; (define (process-collisions s) s)
...
;; Removes any invaders that collide with a missile.
(check-expect (process-collisions-invaders (list I1) empty) (list I1))
(check-expect (process-collisions-invaders (list I1) (list M2)) empty)

;(define (process-collisions-invaders loi lom) loi)
...
;; Checks if Invader is colliding with any missile and removes it from ListOfInvader.
(check-expect (remove-colliding-invader empty (make-invader 150 150 10) empty) (list (make-invader 150 150 10)))
(check-expect (remove-colliding-invader (list (make-missile 150 150)) (make-invader 150 150 10) empty) empty)
...
;; Returns true if the Invader is colliding with a missile.
(check-expect (invader-collides? (list (make-missile 150 150)) (make-invader 150 150 10))
              true)
(check-expect (invader-collides? (list (make-missile 10 10)
                                       (make-missile 50  50)
...
(check-expect (advance-invaders (make-invader-struct INVADE-RATE empty))
              (make-invader-struct (- INVADE-RATE 1) empty))
(check-random (advance-invaders (make-invader-struct 0 empty))
              (make-invader-struct (random INVADE-RATE)
                                   (list (make-invader (random WIDTH)
                                                       0
                                                       (cond [(> (random 10) 5) INVADER-X-SPEED]
                                                             [else (* -1 INVADER-X-SPEED)])))))
...
;; Increment invade countdown timer by one tick and reset it back to INVADE-RATE when
;; it reaches 0.
(check-expect (process-countdown 1) 0)
(check-expect (process-countdown INVADE-RATE) (- INVADE-RATE 1))
(check-random (process-countdown 0) (random INVADE-RATE))
...
(check-expect (process-invader I1) (make-invader (+ (invader-x I1) (invader-dx I1))
                                                 (+ (invader-y I1) INVADER-Y-SPEED)
                                                 (invader-dx I1)))
(check-expect (process-invader I4) (make-invader WIDTH
                                                 (+ (invader-y I4) INVADER-Y-SPEED)
                                                 (* -1 (invader-dx I4))))
(check-expect (process-invader I5) (make-invader 0
...
(check-expect (filter-missiles M1 empty) (list M1))
(check-expect (filter-missiles M4 empty) empty)
(check-expect (filter-missiles M2 (list M1)) (list M2 M1))
(check-expect (filter-missiles M4 (list M1 M2)) (list M1 M2))
...
(check-expect (advance-tank T0) (make-tank (+ (tank-x T0) (* (tank-dir T0) TANK-SPEED))
                                           (tank-dir T0)))
